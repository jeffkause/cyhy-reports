...
class ReportGenerator(object):
    ...
    def __fetch_owner_snapshots(self):
        ...
        self.__snapshots = [
            s
            for s in self.__cyhy_db.SnapshotDoc.find({"owner": self.__owner})
            .sort([("end_time", -1)])
        ...
        if self.__snapshot_id:
            while self.__snapshot_id != self.__snapshots[0]["_id"]:
                ...
                if len(self.__snapshots) == 0:
                    raise Exception(
                        "Could not find requested snapshot %s for %s"
                        % (self.__snapshot_id, self.__owner)
                    )
        self.__no_history = len(self.__snapshots) == 1

    def __fetch_latest_snapshots(self):
        """returns latest snapshots for all owners"""
        self.__latest_snapshots = [
            s for s in self.__cyhy_db.SnapshotDoc.find({"latest": True})
        ]
        ...
        if self.__anonymize:
            ...
            for contact in self.__results["owner"]["agency"]["contacts"]:
                if contact["type"] == POC_TYPE.TECHNICAL:
                    ...
                    contact["email"] = "distro_poc_{}@sample.org".format(
                        distro_poc_count
                    )
                    contact["phone"] = ""
                    distro_poc_count += 1
    ...
    def __load_tickets(self, snapshot_oids):
        ...
        tickets = list(
            self.__cyhy_db.TicketDoc.find(
                {
                    "source": "nessus",
                    "snapshots": {"$in": snapshot_oids},
    ...
    def __load_detected_closed_tickets(self, start_date, end_date):
        ...
        ss0_owners = [self.__snapshots[0]["owner"]] + self.__snapshots[0].get(
        ...
        for t in tickets:
            ...
            if (
                t["last_detected"] > start_date and t["last_detected"] <= end_date
            ):  # This is a ticket we want
    ...
    def __load_false_positive_tickets(self):
        ...
        for t in tickets:
            ...
            t["fp_expiration_date"] = t["fp_expiration_date"].replace(
                hour=0, minute=0, second=0, microsecond=0
            )  # we only display the date in the report; getting rid of the time makes our sort below more useful
            t["expiring_soon"] = t[
                "fp_expiration_date"
            ] < self.__generated_time + datetime.timedelta(
                days=FALSE_POSITIVE_EXPIRING_SOON_DAYS
            )
        tickets.sort(
            key=lambda t: (t["fp_expiration_date"], -t["severity"], t["time_opened"])
        )
    ...
    def __load_ticket_age_data(self, start_date, severity, graph_bucket_cutoff_days):
        ...
        if len(tix):
            ...
            for start_of_day, values in results_df.iterrows():
                end_of_day = (
                    start_of_day + np.timedelta64(1, "D") - np.timedelta64(1, "ns")
                )
    ...
    def __load_risky_services_tickets(self, snapshot_oid):
        ...
        for t in tickets:
            ...
            if self.__no_history or t["time_opened"] > previous_snapshot_timestamp:
    ...
    def __risky_services_metrics(self, tickets):
        ...
        for category in risky_service_categories:
            risky_service_metrics[category] = {"count": 0, "any_newly_opened": False}
    ...
    def __vulnerability_occurrence(self, tickets):
        ...
        if df.empty:
            return DataFrame(
                None, columns=["cvss_base_score", "plugin_name", "severity", "count"]
            )
        grouper = df.groupby(["cvss_base_score", "name", "severity"], as_index=False)
        s = (
            grouper.size()
        )  # works in pandas 0.17.1; in pandas 0.16.2 we did: s = grouper.severity.count()
    ...
    def __run_queries(self):
        ...
        if self.__snapshots[0].get("descendants_included"):
            self.__results["ss0_descendant_snapshots"] = [
                s
                for s in self.__cyhy_db.SnapshotDoc.find(
                    {"parents": ss0_snapshot_oid, "_id": {"$ne": ss0_snapshot_oid}}
                ).sort([("owner", 1)])
            ]
        ...
        if self.__no_history:
            ...
            ss1_snapshot_oid = []
            self.__results[
                "recently_detected_closed_tickets"
            ] = self.__load_detected_closed_tickets(
                self.__snapshots[0]["start_time"], self.__generated_time
            )
        else:
            ...
            self.__results[
                ...
                self.__snapshots[1]["end_time"], self.__generated_time
            )

        self.__results["owner"] = self.__cyhy_db.requests.find_one(
            {"_id": self.__owner}
        )
        ...
        results = database.run_pipeline_cursor(
            queries.ip_geoloc_pl(ss0_owners), self.__cyhy_db
        )
        database.id_expand(results)
        ...
        ss0_host_scans = list(
            self.__cyhy_db.host_scans.aggregate(
                [
                    {"$match": {"snapshots": ss0_snapshot_oid}},
                    {
                        "$project": {
                            "_id": 0,
                            "owner": 1,
        ...
        results = self.__cyhy_db.snapshots.find(
            {"latest": True},
            {"_id": 0, "owner": 1, "cvss_average_all": 1, "cvss_average_vulnerable": 1},
        )
        self.__results["all_cvss_scores"] = [i for i in results]

        results = database.run_pipeline_cursor(
            queries.host_latest_scan_time_span_pl(ss0_owners), self.__cyhy_db
        )
        if results:
            if results[0]["start_time"] < self.__snapshots[0]["start_time"]:
                self.__results["address_scan_start_date"] = results[0]["start_time"]
            else:
                self.__results["address_scan_start_date"] = self.__snapshots[0][
                    "start_time"
                ]
        ...
        results = database.run_pipeline_cursor(
            queries.host_latest_vulnscan_time_span_pl(ss0_owners), self.__cyhy_db
        )
        if results:
            self.__results["vuln_scan_start_date"] = results[0]["start_time"]
            self.__results["vuln_scan_end_date"] = results[0]["end_time"]
        else:
            self.__results["vuln_scan_start_date"] = self.__results[
                "vuln_scan_end_date"
            ] = None

        self.__results["earliest_snapshot_start_time"] = list(
            self.__cyhy_db.SnapshotDoc.collection.find(
                {"owner": {"$in": ss0_owners}}, {"start_time": 1}
            )
            .sort([("start_time", 1)])
            .limit(1)
        )[0]["start_time"]

        critical_ticket_date_cutoff = self.__generated_time - datetime.timedelta(
            days=CRITICAL_AGE_OVER_TIME_CUTOFF_DAYS
        )
        ...
        if self.__results["earliest_snapshot_start_time"] > critical_ticket_date_cutoff:
            ticket_age_start_date = self.__results[
                "earliest_snapshot_start_time"
            ].replace(
                tzinfo=tz.tzutc()
            )  # Explicitly set time zone to UTC
        else:
            ticket_age_start_date = critical_ticket_date_cutoff
        self.__results["critical_ticket_age_data"] = self.__load_ticket_age_data(
            ticket_age_start_date, 4, TICKET_AGE_BUCKET_CUTOFF_DAYS
        )
        ...
        if self.__results.get("ss0_descendant_snapshots"):
            ...
            for snap in self.__results["ss0_descendant_snapshots"]:
                ...
                addresses_scanned_percent = int(
                    safe_percent(snap["addresses_scanned"], address_count, 0)
                )
                vuln_host_percent = int(
                    safe_percent(snap["vulnerable_host_count"], snap["host_count"], 0)
                )
                (
                    tix_days_to_close,
                    tix_days_open,
                ) = self.__vulnerability_mitigation_performance(snap)

                if self.__anonymize:
                    snap["owner"] = "SUB_ORG"

                self.__results["ss0_descendant_data"].append(
                    {
                        "owner": snap["owner"],
                        "address_count": address_count,
                        "addresses_scanned": snap["addresses_scanned"],
                        "addresses_scanned_percent": addresses_scanned_percent,
                        "host_count": snap["host_count"],
                        "vulnerable_host_count": snap["vulnerable_host_count"],
                        "vuln_host_percent": vuln_host_percent,
        ...
        self.__results["is_suborg"] = not (
            self.__snapshots[0]["_id"] in self.__snapshots[0]["parents"]
        )
        ...
        if (
            ...
            if len(cert_counts_groups) > 0:
                ...
                certs["certs_issued_last_thirty_days_count"] = cert_counts[
                    "issued_in_last_thirty_days"
                ]

                # Get a count of all certs issued for this
                # organization in the last 7 days
                certs["certs_issued_last_seven_days_count"] = cert_counts[
                    "issued_in_last_seven_days"
                ]
                ...
                certs["certs_expired_last_seven_days_count"] = cert_counts[
                    "expired_in_last_seven_days"
                ]

                # Get a count of all certs for this organization that
                # expire in the next 7 days
                certs["certs_expire_next_seven_days_count"] = cert_counts[
                    "expire_in_next_seven_days"
                ]

                # Get a count of all certs for this organization that
                # expired in the last 30 days
                certs["certs_expired_last_thirty_days_count"] = cert_counts[
                    "expired_in_last_thirty_days"
                ]

                # Get a count of all certs for this organization that
                # expire in the next 30 days
                certs["certs_expire_next_thirty_days_count"] = cert_counts[
                    "expire_in_next_thirty_days"
                ]
            else:
        ...
        if isinstance(data, dict):
            for k, v in data.items():
                ...
                if isinstance(v, basestring):
                    ...
                    cve_ids = set(
                        re.findall(CVE_ID_RE, v)
                    )  # Search for strings like 'CVE-####-#######'
                    if cve_ids:
                        for (
                            cve
                        ) in cve_ids:  # LaTeX href format:  \href{<URL>}{<Link text>}
    ...
    def __vulnerability_mitigation_performance(self, snapshot):
        ...
        if tix_msec_to_close:
            ...
            if (
                self.__results["earliest_snapshot_start_time"]
                > tix_msec_to_close["tix_closed_after_date"]
            ):
                tix_days_to_close["tix_closed_after_date"] = self.__results[
                    "earliest_snapshot_start_time"
                ]
            else:
                tix_days_to_close["tix_closed_after_date"] = tix_msec_to_close[
                    "tix_closed_after_date"
                ]
            for severity in ["critical", "high", "medium", "low"]:
                for metric in ["median", "max"]:
                    if tix_msec_to_close[severity][metric]:
                        tix_days_to_close[severity][metric] = int(
                            safe_divide(
                                tix_msec_to_close[severity][metric],
                                MSEC_IN_A_DAY,
                                precision=0,
                            )
                        )
                    ...
                    if tix_msec_open[severity][metric]:
                        tix_days_open[severity][metric] = int(
                            safe_divide(
                                tix_msec_open[severity][metric],
                                MSEC_IN_A_DAY,
                                precision=0,
                            )
                        )
    ...
    def __calc_vuln_mitigation_perf_for_all_snapshots(self):
        for snap in self.__snapshots:
            (
                snap["tix_days_to_close"],
                snap["tix_days_open"],
            ) = self.__vulnerability_mitigation_performance(snap)
        self.__results["ss0_tix_days_to_close"] = self.__snapshots[0][
            "tix_days_to_close"
        ]
    ...
    def __table_vulnerabilities_by_severity(self):
        ...
        for sev, count in dict(ss0["unique_vulnerabilities"]).iteritems():
            ss0["unique_vulnerabilities"][sev + "_pct"] = int(
                safe_percent(
                    ss0["unique_vulnerabilities"][sev],
                    ss0["unique_vulnerabilities"]["total"],
                    precision=0,
                )
            )
        for sev, count in dict(ss0["vulnerabilities"]).iteritems():
            ss0["vulnerabilities"][sev + "_pct"] = int(
                safe_percent(
                    ss0["vulnerabilities"][sev],
                    ss0["vulnerabilities"]["total"],
                    precision=0,
                )
            )
    ...
    def __table_vulnerability_occurrence(self):
        ...
        if len(df):  # handle no vulnerabilties
            df.sort_values(
                by=["cvss_base_score", "count", "plugin_name"],
                ascending=[0, 0, 1],
                inplace=True,
            )
    ...
    def __table_top_common_services(self):
        ...
        dfservices = pd.merge(
            static, dfservices, left_index=True, right_index=True, how="outer"
        )
    ...
    def __table_vulnerability_history(self):
        ...
        df = pd.merge(
            ...
            df1,
            how="outer",
            on=["plugin_name", "cvss_base_score", "severity"],
            suffixes=("_0", "_1"),
        )
        ...
        df.sort_values(
            by=["severity", "percent_change"], ascending=[0, 0], inplace=True
        )
        d = self.__dataframe_to_dicts(df)
        self.__convert_levels_to_text(d, "severity")
    ...
    def __table_vulnerability_deltas(self):
        ...
        for df in (df0, df1):
            ...
            for col in ("time_opened", "time_closed", "last_detected"):
                df[col] = pd.to_datetime(
                    df[col], utc=True
                )  # Convert column to pandas Timestamp (datetime64[ns, UTC] or datetime64[ns])
                if (
                    str(df[col].dtype) != "datetime64[ns, UTC]"
                ):  # In some cases, 'time_closed' will already be localized to UTC (i.e. when initial df['time_closed'] had no 'None' values)
                    df[col] = df[col].dt.tz_localize(
                        "UTC"
                    )  # Localize to UTC time zone (dtype: 'datetime64[ns, UTC]'), since fillna above removed tz info

        counts_0 = Series()
        if len(df0):
            counts_0 = df0.groupby(
                [
                    "owner",
                    "plugin_name",
                    "ip",
        ...
        df_new.drop(
            "time_closed", axis=1, inplace=True
        )  # New tickets don't have time_closed, so drop it
        df_new.sort_values(
        ...
        if len(df_resolved):
            df_resolved["days_to_close"] = (
                df_resolved["time_closed"] - df_resolved["time_opened"]
            ) / pd.Timedelta(days=1)
    ...
    def __generate_scope_attachment(self):
        ...
        with open("scope.csv", "wb") as out_file:
            ...
            for net in data:
                if self.__anonymize:
                    row = {
                        "cidr": re.sub(IPV4_ADDRESS_RE, ANONYMOUS_IPV4, str(net.cidr)),
                        "first": re.sub(
                            IPV4_ADDRESS_RE, ANONYMOUS_IPV4, str(IPAddress(net.first))
                        ),
                        "last": re.sub(
                            IPV4_ADDRESS_RE, ANONYMOUS_IPV4, str(IPAddress(net.last))
                        ),
                        "count": net.size,
                    }
    ...
    def __generate_sub_org_summary_attachment(self):
        ...
        if "ss0_descendant_data" in self.__results:
            header_fields = (
                ...
                "hosts_detected",
                "hosts_vulnerable",
                "hosts_vulnerable_percent",
                "critical_vulns_detected",
                "high_vulns_detected",
                "medium_vulns_detected",
                "low_vulns_detected",
                "services_detected",
                "median_days_to_mitigate_criticals",
                "median_days_to_mitigate_highs",
                "median_days_to_mitigate_mediums",
                "median_days_to_mitigate_lows",
                "median_days_currently_active_criticals",
                "median_days_currently_active_highs",
                "median_days_currently_active_mediums",
                "median_days_currently_active_lows",
            )
            data_fields = (
                "owner",
                "address_count",
                ...
                "vulnerabilities.low",
                "port_count",
                "tix_days_to_close.critical.median",
                "tix_days_to_close.high.median",
                "tix_days_to_close.medium.median",
                "tix_days_to_close.low.median",
                "tix_days_open.critical.median",
                "tix_days_open.high.median",
                "tix_days_open.medium.median",
            ...
            with open("sub-org-summary.csv", "wb") as out_file:
                ...
                for row in self.__results["ss0_descendant_data"]:
                    for severity in ("critical", "high", "medium", "low"):
                        row["vulnerabilities." + severity] = row["vulnerabilities"][
                            severity
                        ]
                        row["tix_days_to_close." + severity + ".median"] = row[
                            "tix_days_to_close"
                        ][severity]["median"]
                        row["tix_days_open." + severity + ".median"] = row[
                            "tix_days_open"
                        ][severity]["median"]
                    data_writer.writerow(row)
                ...
                ss0["addresses_scanned_percent"] = int(
                    safe_percent(ss0["addresses_scanned"], ss0["address_count"], 0)
                )
                ss0["vuln_host_percent"] = int(
                    safe_percent(ss0["vulnerable_host_count"], ss0["host_count"], 0)
                )
                for severity in ("critical", "high", "medium", "low"):
                    ss0["vulnerabilities." + severity] = ss0["vulnerabilities"][
                        severity
                    ]
                    ss0["tix_days_to_close." + severity + ".median"] = self.__results[
                        "ss0_tix_days_to_close"
                    ][severity]["median"]
                    ss0["tix_days_open." + severity + ".median"] = self.__results[
                        "ss0_tix_days_open"
                    ][severity]["median"]
    ...
    def __generate_days_to_mitigate_attachment(self):
        header_fields = (
            ...
            "median_days_to_mitigate_highs",
            "max_days_to_mitigate_highs",
            "median_days_to_mitigate_mediums",
            "max_days_to_mitigate_mediums",
            "median_days_to_mitigate_lows",
            "max_days_to_mitigate_lows",
            "calculated_with_vulns_mitigated_since",
        )
        data_fields = (
            "report_date",
            "tix_days_to_close.critical.median",
            "tix_days_to_close.critical.max",
            "tix_days_to_close.high.median",
            "tix_days_to_close.high.max",
            "tix_days_to_close.medium.median",
            "tix_days_to_close.medium.max",
            "tix_days_to_close.low.median",
            "tix_days_to_close.low.max",
            "tix_closed_after_date",
        )
        with open("days-to-mitigate.csv", "wb") as out_file:
            ...
            for snap in self.__snapshots:
                ...
                snap["tix_closed_after_date"] = snap["tix_msec_to_close"][
                    "tix_closed_after_date"
                ].strftime("%Y-%m-%d")
                for severity in ("critical", "high", "medium", "low"):
                    snap["tix_days_to_close." + severity + ".median"] = snap[
                        "tix_days_to_close"
                    ][severity]["median"]
                    snap["tix_days_to_close." + severity + ".max"] = snap[
                        "tix_days_to_close"
                    ][severity]["max"]
    ...
    def __generate_days_currently_active_attachment(self):
        header_fields = (
            "report_date",
            "median_days_currently_active_criticals",
            "max_days_currently_active_criticals",
            "median_days_currently_active_highs",
            "max_days_currently_active_highs",
            "median_days_currently_active_mediums",
            "max_days_currently_active_mediums",
            "median_days_currently_active_lows",
            "max_days_currently_active_lows",
        )
        data_fields = (
            "report_date",
            "tix_days_open.critical.median",
            "tix_days_open.critical.max",
            "tix_days_open.high.median",
            "tix_days_open.high.max",
            "tix_days_open.medium.median",
            "tix_days_open.medium.max",
            "tix_days_open.low.median",
        ...
        with open("days-currently-active.csv", "wb") as out_file:
            ...
            for snap in self.__snapshots:
                ...
                for severity in ("critical", "high", "medium", "low"):
                    snap["tix_days_open." + severity + ".median"] = snap[
                        "tix_days_open"
                    ][severity]["median"]
                    snap["tix_days_open." + severity + ".max"] = snap["tix_days_open"][
                        severity
                    ]["max"]
                data_writer.writerow(snap)
    ...
    def __generate_mustache_json(self, filename):
        ...
        if self.__no_history:
            ss1 = ss0
            calc["host_count_percent"] = calc["vulnerable_host_count_percent"] = calc[
                "unique_operating_systems_percent"
            ] = calc["unique_services_percent"] = calc[
                "unique_vulnerabilities_percent"
            ] = "-"
            calc["vuln_host_count_pct_flat_flag"] = True
            calc["vuln_host_count_pct_change_int"] = 0
        else:
            ss1 = self.__snapshots[1]
            calc["host_count_percent"] = self.__percent_change(
                ss1["host_count"], ss0["host_count"]
            )
            calc["vulnerable_host_count_percent"] = self.__percent_change(
                ss1["vulnerable_host_count"], ss0["vulnerable_host_count"]
            )
            ...
            if calc["vuln_host_count_pct_flat_flag"]:
                ...
                calc["vuln_host_count_pct_change_int"] = abs(
                    int(round(calc["vulnerable_host_count_percent"], 0))
                )

            calc["unique_operating_systems_percent"] = self.__percent_change(
                ss1["unique_operating_systems"], ss0["unique_operating_systems"]
            )
            calc["unique_services_percent"] = self.__percent_change(
                len(ss1["services"]), len(ss0["services"])
            )
            calc["unique_vulnerabilities_percent"] = self.__percent_change(
                ss1["unique_vulnerabilities"]["total"],
                ss0["unique_vulnerabilities"]["total"],
            )
        result["ss1"] = ss1
        # create items for _up, _down, or _flat (used in mustache conditionals)
        calc.update(
            self.__udf_calc("address_count", len(ss1.networks), len(ss0.networks))
        )
        calc.update(
            self.__udf_calc(
                "addresses_scanned", ss1["addresses_scanned"], ss0["addresses_scanned"]
            )
        )
        ...
        calc.update(
            self.__udf_calc(
                "vuln_host_count",
                ss1["vulnerable_host_count"],
                ss0["vulnerable_host_count"],
            )
        )
        ...
        calc.update(
            self.__udf_calc(
                "vuln_critical",
                ss1["vulnerabilities"]["critical"],
                ss0["vulnerabilities"]["critical"],
            )
        )
        calc.update(
            self.__udf_calc(
                "vuln_high",
                ss1["vulnerabilities"]["high"],
                ss0["vulnerabilities"]["high"],
            )
        )
        calc.update(
            self.__udf_calc(
                "vuln_medium",
                ss1["vulnerabilities"]["medium"],
                ss0["vulnerabilities"]["medium"],
            )
        )
        calc.update(
            self.__udf_calc(
                "vuln_low", ss1["vulnerabilities"]["low"], ss0["vulnerabilities"]["low"]
            )
        )
        calc.update(
            self.__udf_calc(
                "vuln_total",
                ss1["vulnerabilities"]["total"],
                ss0["vulnerabilities"]["total"],
            )
        )
        ...
        calc["addresses_scanned_percent"] = int(
            safe_percent(ss0["addresses_scanned"], calc["address_count"], 0)
        )
        if self.__results["address_scan_start_date"]:
            calc["address_scan_start_date_tex"] = self.__results[
                "address_scan_start_date"
            ].strftime("{%d}{%m}{%Y}")
            calc["address_scan_end_date_tex"] = self.__results[
                "address_scan_end_date"
            ].strftime("{%d}{%m}{%Y}")
        if self.__results["vuln_scan_start_date"]:
            calc["vuln_scan_start_date_tex"] = self.__results[
                "vuln_scan_start_date"
            ].strftime("{%d}{%m}{%Y}")
            calc["vuln_scan_end_date_tex"] = self.__results[
                "vuln_scan_end_date"
            ].strftime("{%d}{%m}{%Y}")
        if self.__results["ss0_tix_days_to_close"]:
            calc["ss0_tix_closed_after_date_tex"] = self.__results[
                "ss0_tix_days_to_close"
            ]["tix_closed_after_date"].strftime("{%d}{%m}{%Y}")
            calc["ss0_tix_days_to_close"] = self.__results["ss0_tix_days_to_close"]
            calc["ss0_tix_days_open"] = self.__results["ss0_tix_days_open"]
        calc["vuln_host_percent"] = int(
            safe_percent(ss0["vulnerable_host_count"], ss0["host_count"], 0)
        )
        calc["ss0_unique_services_count"] = len(ss0["services"])
        calc["ss1_unique_services_count"] = len(ss1["services"])
        calc["average_vulnerabilities_per_host"] = safe_divide(
            ss0["vulnerabilities"]["total"], ss0["host_count"], 2
        )
        ...
        result["density"] = self.__vulnerability_density(
            self.__results["tickets_0"]
        ).to_dict()
        result["top_operating_systems"] = self.__results["top_operating_systems"]
        result["top_services"] = self.__results["top_services"]
        result["top_10_vulnerability_occurrence"] = self.__results[
            "top_10_vulnerability_occurrence"
        ]

        result["top_common_services_static"] = self.__results[
            "top_common_services_static"
        ]
        result["top_common_services_other"] = self.__results[
            "top_common_services_other"
        ]
        other_services = [i["index"] for i in result["top_common_services_other"]]
        result["top_common_services_other_english"] = self.__to_oxford_list(
            other_services, " was", " were"
        )
        result["top_10_risky_hosts"] = self.__dataframe_to_dicts(
            self.__top_risky_hosts(self.__results["tickets_0"])[:10]
        )
        ...
        result["brand_new_vulnerabilities"] = self.__results[
            "brand_new_vulnerabilities"
        ]
        ...
        result["redetected_vulnerabilities"] = self.__results[
            "redetected_vulnerabilities"
        ]
        ...
        result["resolved_vulnerability_counts"] = self.__results[
            "resolved_vulnerability_counts"
        ]

        result["recently_detected_closed_tickets"] = self.__results[
            "recently_detected_closed_tickets"
        ]
        ...
        for t in result["false_positive_tickets"]:
            ...
            t["fp_expiration_date_tex"] = t["fp_expiration_date"].strftime(
                "{%d}{%m}{%Y}"
            )
            if t["expiring_soon"]:
                result["expiring_soon_false_positive_tickets"].append(
                    t.copy()
                )  # append a copy otherwise chars get escaped twice in latex_escape_structure_make_cve_urls()
    ...
    def __log_report(self):
        ...
        report["snapshot_oid"] = [
            self.__snapshots[0]["_id"]
        ]  # This is a list for backwards-compatibility; see changes in CYHY-258
